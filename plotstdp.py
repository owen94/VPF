...
#
# plt.xlabel('Spikes timing (ms)', fontsize = 20)
# plt.ylabel('Weight updates $\\frac{ \\Delta W_{ij}}{W_{ij}}$', fontsize = 20)
#
#
#
# #plt.show()
# plt.savefig( '../DBN_results/STDP.pdf', bbox_inches='tight')


x = np.arange(-0.6, -0.008, 0.001)
y = - 60 * np.exp(10*x)
print(y)
...
#plt.xlabel('Expected time to post-synaptic spike $1/\\delta_{j}$', fontsize = 20)
plt.xlabel('Spikes timing (ms)', fontsize = 20)
plt.ylabel('Weight updates $\\Delta W_{ij} $', fontsize = 20)
...
#plt.show()
plt.savefig( '../DBN_results/weightsupdate.pdf', bbox_inches='tight')
